...
```
t = (performance.now() - lastTickTime) / expectedTickInterval
t = Math.min(1, t)  // clamp
```
...
//   const sound = sounds.get(name)?.cloneNode(true) as HTMLAudioElement
//   sound.volume = volume
...
// Use CSS only, no external image assets required:
