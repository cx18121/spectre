...
must_haves:
  truths:
    ...
    - "speed is computed via computeWristPeakSpeed from fps/src/lib/velocity.ts (not a model output)"
  ...
  key_links:
    ...
    - from: "fps/src/hooks/usePunchClassifier.ts"
      ...
      via: "import { computeWristPeakSpeed, LANDMARK } from '../lib/velocity'"
      pattern: "computeWristPeakSpeed"
    - from: "fps/src/hooks/usePunchClassifier.ts"
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 7 (SC4 proxy): normalizeWindow preserves z-coordinates — a window built from keypoints with wrist z=0.5 (arm extended toward camera) has non-zero z in the normalized flat tensor, confirming z reaches the model input (full z-disambiguation validation is deferred to post-training empirical evaluation)
  </behavior>
  <action>
